...
void takeInput() {
    ...
    // Se não existirem cidades (ou existirem cidades negativas), o vendedor não pode viajar.
    if(MAX_CIDADES <= 0) {
        showError(422);
    }
    ...
    ponto coordenadas[100];         string coordenadas_existentes;
    ...
    removeCharsFromString(coordenadas_existentes, const_cast<char*>("()"), coordenadas_existentes.length());
    ...
    removeCharsFromString(s_coordenada_inicial, const_cast<char*>("()"), s_coordenada_inicial.length());
...
void putCoordinatesInMatrix(int MAX_CIDADES, ponto *coordenadas) {
    ...
    for(int h = 0; h < MAX_CIDADES; h++) {
        for(int k = 0; k < MAX_CIDADES; k++) {
            ...
            /*  Se o i for igual a k, então estamos tratando de uma relação de si para si, e como a distância será 0, não mudaremos o que
                já está definido */
 
            if(h != k) {    // Se não for uma posição de relação si para si, e ainda não tiver sido percorrido, teremos que preencher a
                if(pontosDistancia[h][k] == 0) { // a mesma distância para pontosDistancia[i][k] e [k][i]
...
void showError(int codigoErro) {
    switch(codigoErro) {
        case 400:
            ...
            break;
        case 404:
            cout << "Entrada inválida - A coordenada inserida nao pertence ao universo (404)" << "\n";
            break;
        case 422:
            cout << "Entrada inválida - numero negativo ou nulo de cidades (422)" << "\n";
            break;
        default:
